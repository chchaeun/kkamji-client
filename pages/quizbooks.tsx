...
function QuizbooksPage() {
  ...
  const { data: quizbooks } = useQuery<IQuizbook[]>(["quizbooks"], () =>
    fetchQuizbooks(typeof week === "string" ? week : "")
  );

  const onQuizBookClick = (quizbook: IQuizbook) => {
    router.push(`/quizbook/${quizbook.quizbookId}`);
  };
  return (
    <div className="grid grid-cols-5 gap-4 w-full m-auto sm:flex">
      <div className="col-start-1 flex justify-center mt-10">
        <SideNav title={navTitle} elements={navElements} />
      </div>
      <div className="col-start-2 col-span-3 flex flex-col gap-10  sm:gap-7 sm:w-4/5 h-screen bg-white py-10 px-20 sm:m-auto">
